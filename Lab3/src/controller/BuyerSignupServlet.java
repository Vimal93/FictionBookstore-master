	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/** The proceeding code block correlates to the second button */
			...
			if ( username.length() == 0 ) {
				...
				try {
					if ( AuthDAO.isUserNameAvailable(username) ) {
						int signupStatus = AuthDAO.enterNewUser(username, password, 1); //add extra parameters in AuthDAO method
						if (signupStatus > 0) {
							AuthDAO.enterUserData(signupStatus, firstName, middleName, lastName, email);
							msg = "<font color=\"red\"> Account Created! You are now an authorized Buyer.</font><br/>";
							url = "/login.jsp";
						}
